...
const Dependency = function (name, tree) {
    this.tree = tree;
    this.name = name;
...
const Tree = function (baseUrl, noRename, hashAlgo, salt) {
...
Tree.prototype.process = function () {
    const levels = _.groupBy(this.nodes, node => node.height());
    ...
    for (let height = 0, maxHeight = _.size(levels) - 1; height <= maxHeight; height++) {
        ...
        for (let i = 0, len = level.length; i < len; i++) {
            ...
            if (_.includes(this.noRename, name))
                continue;
            ...
            node.name = normalizePath(
                `${path.dirname(name)}/${path.basename(name, node.ext)}.${node.hash}${node.ext}`
            );
...
Tree.prototype.stats = function () {
    return Object.keys(this.nodes).reduce((stats, name) => {
...
Dependency.prototype.height = function () {
    ...
    if (dependencies.length)
        result = 1 + Math.max.apply(Math, _.invokeMap(dependencies, 'height'));
    else
        result = 0;
